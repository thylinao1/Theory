...
By memorylessness, $V - t^{\ast}$ is Exponential($\lambda$), independent of the past. The elapsed time $t^{\ast} - U$ is also Exponential($\lambda$) by time-reversal symmetry. Therefore:

$$
E[V - U] = E[V - t^{\ast}] + E[t^{\ast} - U] = \frac{1}{\lambda} + \frac{1}{\lambda} = \frac{2}{\lambda}
$$
