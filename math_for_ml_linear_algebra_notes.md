...
$$\vec{h} = \begin{bmatrix} 120 \\\ 2 \\\ 1 \\\ 150000 \end{bmatrix} \quad \text{(area in m}^2\text{, bedrooms, bathrooms, price in euros)}$$
...
$$\begin{bmatrix} 3 \\\ 2 \end{bmatrix} \cdot \begin{bmatrix} -1 \\\ 2 \end{bmatrix} = 3(-1) + 2(2) = -3 + 4 = 1$$
...
$$A\vec{r} = \begin{bmatrix} a & b \\\ c & d \end{bmatrix}\begin{bmatrix} x \\\ y \end{bmatrix} = \begin{bmatrix} ax + by \\\ cx + dy \end{bmatrix}$$
...
The columns of a transformation matrix tell us where the basis vectors go. The first column is where $\hat{e}_1$ lands, and the second column is where $\hat{e}_2$ lands.
...
Can be written as: $A\vec{x} = \vec{d}$
...
To transform a vector from one basis to another, construct matrix $B$ whose columns are the new basis vectors expressed in the original coordinates.

**Transformation:**
- $B \cdot \vec{v}_{\text{new basis}} = \vec{v}_{\text{original}}$ (new basis vector to original coordinates)
- $B^{-1} \cdot \vec{v}_{\text{original}} = \vec{v}_{\text{new basis}}$ (original coordinates to new basis)
...
**Example:** For $A = \begin{bmatrix} 1 & 0 \\\ 0 & 2 \end{bmatrix}$:

- $\lambda_1 = 1$: eigenvector $\vec{x} = t(1, 0)^T$ (any horizontal vector)
- $\lambda_2 = 2$: eigenvector $\vec{x} = t(0, 1)^T$ (any vertical vector)
...
$$D = \begin{bmatrix} a & 0 & 0 \\\ 0 & b & 0 \\\ 0 & 0 & c \end{bmatrix} \implies D^n = \begin{bmatrix} a^n & 0 & 0 \\\ 0 & b^n & 0 \\\ 0 & 0 & c^n \end{bmatrix}$$
...
**Eigenvectors:** $(1, 0)^T$ with $\lambda = 1$, and $(1, 1)^T$ with $\lambda = 2$

$$C = \begin{bmatrix} 1 & 1 \\\ 0 & 1 \end{bmatrix}, \quad D = \begin{bmatrix} 1 & 0 \\\ 0 & 2 \end{bmatrix}$$

Then:

$$T^2 = CD^2C^{-1} = \begin{bmatrix} 1 & 3 \\\ 0 & 4 \end{bmatrix}$$
...
$$L_A = \begin{bmatrix} 0 \\\ 1/3 \\\ 1/3 \\\ 1/3 \end{bmatrix}$$
...
| Concept | Formula |
|---------|---------|
| Dot product | $\vec{r} \cdot \vec{s} = \sum_i r_is_i = \lvert\vec{r}\rvert\lvert\vec{s}\rvert\cos\theta$ |
| Vector length | $\lvert\vec{r}\rvert = \sqrt{\vec{r} \cdot \vec{r}}$ |
| Scalar projection | $\frac{\vec{r} \cdot \vec{s}}{\lvert\vec{r}\rvert}$ |
| Vector projection | $\frac{\vec{r} \cdot \vec{s}}{\lvert\vec{r}\rvert^2}\vec{r}$ |
