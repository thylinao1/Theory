...
**Special cases:**
- **-r** = vector pointing in opposite direction
- **r** + (**-r**) = **0** (zero vector)
...
A coordinate system is defined by basis vectors (typically denoted e1, e2 or i, j).
...
```
r . s = r1*s1 + r2*s2 + ... + rn*sn = SUM(ri * si)
```
...
2. **Distributive over addition:** r . (s + t) = r . s + r . t

3. **Associative with scalar multiplication:** r . (a*s) = a*(r . s)
...
```
|r| = sqrt(r . r) = sqrt(r1^2 + r2^2 + ... + rn^2)
```
...
**Implications:**
- If theta = 0 degrees (same direction): r . s = |r| * |s| (maximum positive)
- If theta = 90 degrees (perpendicular/orthogonal): r . s = 0
- If theta = 180 degrees (opposite directions): r . s = -|r| * |s| (maximum negative)
...
The "shadow" of **s** onto **r**:

```
Scalar projection of s onto r = (r . s) / |r| = |s| * cos(theta)
```

#### Vector Projection

The projection as a vector in the direction of **r**:

```
Vector projection of s onto r = [(r . s) / (r . r)] * r
```
...
Vectors {b1, b2, ..., bn} are linearly independent if no bi can be written as:

```
bi = a1*b1 + a2*b2 + ... + a(i-1)*b(i-1) + a(i+1)*b(i+1) + ... + an*bn
```
...
**Given:** Vector **r** in basis {e1, e2}, new basis {b1, b2} where b1 perpendicular to b2

**To find r in the new basis:**
...
- e1 = (1, 0) transforms to (2, 10)
- e2 = (0, 1) transforms to (3, 1)
...
- Linear combinations: A*(n*r) = n*A*r and A*(r + s) = A*r + A*s
...
Applying transformation A1 followed by A2 is equivalent to multiplying by A2*A1:
...
Matrices don't have to be square. An (m x n) matrix times an (n x p) matrix yields an (m x p) matrix:
...
If A^(-1) exists such that A^(-1) * A = A * A^(-1) = I, then:
...
**Transformation:**
- B * v_bear = v_mine (Bear's vector to my coordinates)
- B^(-1) * v_mine = v_bear (My vector to Bear's coordinates)
...
**Process:**
1. Convert to standard basis: B * v
2. Apply transformation: T * (B * v)
3. Convert back: B^(-1) * T * (B * v)
...
- All vectors are mutually orthogonal: ei . ej = 0 for i != j
- All vectors have unit length: |ei| = 1
...
**Problem:** Reflect a point through a plane defined by vectors v1 and v2.

**Solution:**
1. Use Gram-Schmidt to find orthonormal basis: e1, e2 (in plane), e3 (normal to plane)
2. Build transformation matrix E = [e1 | e2 | e3]
...
| Rotation (not 180 deg) | None (real) | Complex |
| 180 deg rotation | All vectors | -1 |
...
- lambda_1 = 1: eigenvector x = t*(1, 0) (any horizontal vector)
- lambda_2 = 2: eigenvector x = t*(0, 1) (any vertical vector)

### 12.5 Special Cases

**No real eigenvectors:** Rotation matrices (except 180 degrees) have only complex eigenvalues.

**All vectors are eigenvectors:** Uniform scaling and 180 degree rotation.
...
```
T^2 = (C*D*C^(-1)) * (C*D*C^(-1)) = C*D*(C^(-1)*C)*D*C^(-1) = C*D^2*C^(-1)
```
...
**Step 3 - Converge:** When r stops changing, we've found the eigenvector.
...
```
r_(i+1) = d * L * r_i + [(1-d)/n] * 1
```

Where d (approximately 0.85) represents probability of following a link (vs. randomly jumping).
...
| Concept | Formula |
|---------|---------|
| Dot product | r . s = SUM(ri * si) = \|r\| * \|s\| * cos(theta) |
| Vector length | \|r\| = sqrt(r . r) |
| Scalar projection | (r . s) / \|r\| |
| Vector projection | [(r . s) / (r . r)] * r |
...
| 2x2 Inverse | A^(-1) = [1/(a*d - b*c)] * [d, -b; -c, a] |
...
| Concept | Formula |
|---------|---------|
| Eigenvector equation | A * x = lambda * x |
| Characteristic equation | det(A - lambda * I) = 0 |
| Diagonalization | A = C * D * C^(-1) |
| Matrix power | A^n = C * D^n * C^(-1) |
