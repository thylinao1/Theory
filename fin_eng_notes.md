...
1. **Symmetric**: $\Sigma_{ij} = \text{Cov}(X_i, X_j) = \text{Cov}(X_j, X_i) = \Sigma_{ji}$.
2. **Positive semi-definite**: $\mathbf{x}^\top \Sigma \mathbf{x} \geq 0$ for all $\mathbf{x} \in \mathbb{R}^n$.
3. Diagonal entries are non-negative (they are variances).
...
A useful scalar special case: $\text{Var}(aX + bY) = a^2 \text{Var}(X) + b^2 \text{Var}(Y) + 2ab\,\text{Cov}(X, Y)$.
...
- **Case 2:** $W_n = -(2^n - 1)$ (haven't won yet). We bet $2^n$ on the next toss:
  - Win (prob $\frac{1}{2}$): $W_{n+1} = 1$
  - Lose (prob $\frac{1}{2}$): $W_{n+1} = -(2^{n+1} - 1)$
  
  Then $E[W_{n+1} \mid W_n] = \frac{1}{2}(1) + \frac{1}{2}(-(2^{n+1} - 1)) = \frac{1}{2} - \frac{2^{n+1} - 1}{2} = -(2^n - 1) = W_n$.
...
$$X_{n+1} = \begin{cases} k + 1 & \text{with probability } \frac{k}{n+2} \quad \text{(drew red)} \\ k & \text{with probability } \frac{n + 2 - k}{n + 2} \quad \text{(drew green)} \end{cases}$$
...
$$E[W_{t+s} \cdot W_s] = E\!\big[(W_{t+s} - W_s + W_s) \cdot W_s\big]$$

$$= E\!\big[(W_{t+s} - W_s) \cdot W_s\big] + E[W_s^2]$$
...
1. **Positive values:** If $X_t > 0$, then $X_{t+s} > 0$ for all $s > 0$ (since the exponential is always positive). This respects the **limited liability** of stock prices — they cannot go negative.

2. **Returns are independent of price level:** The distribution of $\frac{X_{t+s}}{X_t}$ depends only on $s$, $\mu$, and $\sigma$ — not on $X_t$. This is a desirable property: we generally do not expect stock returns to depend on the current price.
...
4. **Independent return ratios:** The ratios $\frac{X_{t_2}}{X_{t_1}}, \frac{X_{t_3}}{X_{t_2}}, \ldots$ are mutually independent (following from the independent increments of Brownian motion).

5. **Continuous paths:** $X_t$ is continuous in $t$ (it never jumps), inherited from the continuity of Brownian motion.
...
A **vector** is an ordered collection of $n$ real numbers. It can be arranged as a **row vector** $(v_1, v_2, \ldots, v_n)$ or a **column vector** $\begin{pmatrix} v_1 \\ v_2 \\ \vdots \\ v_n \end{pmatrix}$. We write $\mathbf{v} \in \mathbb{R}^n$ to indicate a vector with $n$ real-valued components.
...
**Primal:** $\; \min_{\mathbf{x}} \; \mathbf{c}^\top \mathbf{x} \quad \text{s.t.} \quad A\mathbf{x} \geq \mathbf{b}$

**Dual:** $\; \max_{\mathbf{u}} \; \mathbf{b}^\top \mathbf{u} \quad \text{s.t.} \quad A^\top \mathbf{u} = \mathbf{c}, \; \mathbf{u} \geq \mathbf{0}$

**Weak duality** states that the optimal primal value $P$ is always at least as large as the optimal dual value $D$: $P \geq D$. This gives a useful chain of inequalities: for any primal-feasible $\mathbf{x}$ and dual-feasible $\mathbf{u}$:
...
For the equality-constrained variant: $\min \mathbf{c}^\top \mathbf{x}$ s.t. $A\mathbf{x} = \mathbf{b}$ has dual $\max \mathbf{b}^\top \mathbf{u}$ s.t. $A^\top \mathbf{u} = \mathbf{c}$ (with $\mathbf{u}$ unrestricted in sign).
...
$$\nabla f(\mathbf{x}^*) = \begin{pmatrix} \frac{\partial f}{\partial x_1} \\ \vdots \\ \frac{\partial f}{\partial x_n} \end{pmatrix}\bigg|_{\mathbf{x}^*} = \mathbf{0}$$
...
Setting $\nabla f = \mathbf{0}$: $\; 2x_1 + 3x_2 = 0$ and $3x_1 + 3x_2^2 = 0$. Solving yields two critical points: $(0, 0)$ and $(-\frac{9}{4}, \frac{3}{2})$.

The Hessian is $H = \begin{pmatrix} 2 & 3 \\ 3 & 6x_2 \end{pmatrix}$.
...
Substituting into the constraint $x_1 + x_2 = 12$: $\; \frac{6}{\nu} - 2 = 12$, giving $\nu = \frac{3}{7}$.
...
| Multivariate Normal | $f(\mathbf{x}) \propto \exp(-\frac{1}{2}(\mathbf{x}-\boldsymbol{\mu})^\top\Sigma^{-1}(\mathbf{x}-\boldsymbol{\mu}))$ | Mean–variance analysis, Black–Scholes |
| Martingale Property | $E[X_{n+m} \mid \mathcal{F}_n] = X_n$ | Fair pricing, trading strategies |
| Brownian Motion | $W_{t+s} - W_t \sim N(0, s)$, independent of $\mathcal{F}_t$ | Foundation of continuous-time finance |
